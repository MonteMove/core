...
export class JwtTokenService {
    ...
    constructor(
        ...
        this.maxSessions = Number(this.configService.getOrThrow<string>('MAX_SESSIONS'));
    }

    public async generateTokenPair(userPayload: UserPayload, sessionInfo: SessionInfo): Promise<TokenPair> {
        ...
        const accessToken = this.jwtService.sign(
            { ...userPayload, jti },
            {
                expiresIn: parseInt(this.configService.getOrThrow<string>('JWT_ACCESS_TOKEN_EXPIRES_IN')),
            },
        );
        ...
        const refreshToken = this.jwtService.sign(
            ...
            {
                expiresIn: parseInt(this.configService.getOrThrow<string>('JWT_REFRESH_TOKEN_EXPIRES_IN')),
            },
        );
    ...
    public async validateRefreshToken(refreshToken: string, sessionJti: string): Promise<string | null> {
        try {
    ...
    public async validateAccessToken(accessToken: string): Promise<UserPayload | null> {
        try {
    ...
    public async refreshTokens(
        ...
        const userId = await this.validateRefreshToken(refreshToken, decodedAccess.jti);
