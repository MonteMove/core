...
export class OperationController {
  ...
  @ApiOperation({
    ...
    description:
      'Формирует Excel-отчет с детализацией операций и движением по кошелькам.',
  })
  @ApiResponse({ status: 200, description: 'Отчет сформирован' })
  public async downloadOperationsReport(
    @Query() dto: GetOperationsReportDto,
  ...
  @ApiOperation({
    ...
    description:
      'Формирует Excel-отчет по операциям конвертации с расчетом курса обмена.',
  })
  @ApiResponse({ status: 200, description: 'Отчет сформирован' })
  public async downloadConversionReport(
    @Query() dto: GetConversionReportDto,
  ): Promise<StreamableFile> {
  ...
  @ApiOperation({
    ...
    description:
      'Формирует Excel по текущим остаткам кошельков, с возможностью фильтрации по типу.',
  })
  @ApiResponse({ status: 200, description: 'Отчет сформирован' })
  public async downloadClosingPeriodReport(
    @Query() dto: GetClosingPeriodReportDto,
  ): Promise<StreamableFile> {
  ...
  @ApiOperation({
    ...
    description:
      'Возвращает список операций с поддержкой фильтрации, поиска и пагинации.',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Создаёт новую операцию. Доступно администраторам и модераторам.',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Обновляет данные существующей операции. Доступно администраторам и модераторам.',
  })
  @ApiIdParam('Уникальный идентификатор операции')
  ...
  public async updateOperation(
    ...
    const result = await this.updateOperationUseCase.execute(
      operationId,
      updateOperationDto,
      userId,
    );
  ...
  @ApiOperation({
    ...
    description:
      'Создаёт корректировочную операцию для выравнивания баланса кошелька.',
  })
  ...
  public async adjustmentOperation(
    ...
    const result = await this.adjustmentOperationUseCase.execute(
      adjustmentOperationDto,
      userId,
    );
  ...
  public async deleteOperation(
    ...
    const result = await this.deleteOperationUseCase.execute(
      operationId,
      userId,
    );
