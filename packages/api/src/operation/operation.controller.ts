...
export class OperationController {
    ...
    @ApiResponse({ status: 200, description: 'Список операций успешно получен', type: GetOperationsResponseDto })
    @ApiReadResponses()
    ...
    @ApiResponse({ status: 200, description: 'Операция найдена', type: OperationResponseDto })
    @ApiReadResponses()
    ...
    @ApiResponse({ status: 201, description: 'Операция успешно создана', type: CreateOperationResponseDto })
    @ApiCrudResponses()
    ...
    @ApiResponse({ status: 200, description: 'Операция успешно обновлена', type: UpdateOperationResponseDto })
    @ApiCrudResponses()
    ...
    @HttpCode(HttpStatus.OK)
    @Roles(RoleCode.admin)
    @ApiOperation({ summary: 'Удалить операцию', description: 'Выполняет мягкое удаление операции.' })
    @ApiIdParam('Уникальный идентификатор операции')
    @ApiResponse({ status: 200, description: 'Операция успешно удалена', type: DeleteOperationResponseDto })
    @ApiReadResponses()
