...
export class AdjustmentOperationResponseDto {
    ...
    @ApiProperty({ description: 'Созданная корректировочная операция', type: OperationResponseDto, nullable: true })
    public operation: OperationResponseDto | null;

    @ApiProperty({ description: 'Баланс кошелька до корректировки', example: 4000 })
    public previousAmount: number;

    @ApiProperty({ description: 'Баланс кошелька после корректировки', example: 5000 })
