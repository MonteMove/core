...
export class OperationEntryDto {
    @ApiProperty({ description: 'ID кошелька', example: '123e4567-e89b-12d3-a456-426614174000', format: 'uuid' })
    @IsUUID('4', { message: 'ID кошелька должен быть валидным UUID' })
    public walletId: string;

    @ApiProperty({ description: 'Направление операции', enum: OperationDirection, example: OperationDirection.credit })
...
export class CreateOperationDto {
    @ApiProperty({ description: 'ID типа операции', example: '123e4567-e89b-12d3-a456-426614174100', format: 'uuid' })
    ...
    @ApiProperty({ description: 'Список записей операции', type: [OperationEntryDto] })
