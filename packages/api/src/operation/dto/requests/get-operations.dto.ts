...
export class GetOperationsDto {
    @ApiProperty({ description: 'Поиск по описанию операции', example: 'перевод', required: false })
    @IsOptional()
    ...
    @ApiProperty({ description: 'Фильтр по ID группы конверсии', example: 7, minimum: 1, required: false })
    @IsOptional()
    @Type(() => Number)
    ...
    @ApiProperty({ description: 'Фильтр по направлению записи', enum: OperationDirection, required: false })
    @IsOptional()
    @IsEnum(OperationDirection, { message: 'Неверное направление операции' })
    public direction?: OperationDirection;

    @ApiProperty({ description: 'Минимальная сумма записи', example: 100, minimum: 0, required: false })
    @IsOptional()
    @Type(() => Number)
    ...
    @ApiProperty({ description: 'Максимальная сумма записи', example: 10000, minimum: 0, required: false })
    @IsOptional()
    @Type(() => Number)
    ...
    @ApiProperty({ description: 'Номер страницы (>=1)', example: 1, required: false })
    @IsOptional()
    @Type(() => Number)
    ...
    @ApiProperty({ description: 'Количество элементов на странице (1-100)', example: 10, required: false })
    @IsOptional()
    @Type(() => Number)
