...
export class AdjustmentOperationUseCase {
  ...
  public async execute(
    adjustmentDto: AdjustmentOperationDto,
    userId: string,
  ): Promise<AdjustmentOperationResponse> {
    ...
    const adjustmentDescription =
      description ||
      `Корректировка баланса с ${currentAmount} до ${targetAmount}`;

    return await this.prisma.$transaction(async (tx) => {
      const calculatedAmount =
        await this.walletRecalculationService.getCalculatedWalletAmount(
          tx,
          walletId,
        );
      ...
      if (difference === 0) {
        await this.walletRecalculationService.recalculateWallet(
          tx,
          walletId,
          userId,
        );
      ...
      const direction =
        difference > 0 ? OperationDirection.credit : OperationDirection.debit;
      ...
      const actionType =
        direction === OperationDirection.credit ? 'пополнение' : 'списание';
