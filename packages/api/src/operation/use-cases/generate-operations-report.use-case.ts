...
import { OperationDirection, Prisma, WalletType } from '../../../prisma/generated/prisma';
...
export class GenerateOperationsReportUseCase {
    ...
    public async execute(dto: GetOperationsReportDto): Promise<ReportFile> {
        ...
        const rows = this.buildRows(operations, walletMap, dto.typeUnloading ?? OperationReportWalletFilter.all);
    ...
    private async buildWalletMap(operations: OperationsReportOperation[]): Promise<Map<string, WalletLookupEntry>> {
        ...
        const wallets: { id: string; name: string; walletType: WalletType; currency: { code: string } }[] =
    ...
    private shouldIncludeOperation(
        ...
        const walletTypes = operation.entries
            .map((entry) => walletMap.get(entry.walletId)?.walletType)
            .filter((type): type is WalletType => Boolean(type));
        ...
        if (filter === OperationReportWalletFilter.wnzh) {
            return walletTypes.some((type) => type === WalletType.vnj);
        }

        if (filter === OperationReportWalletFilter.inskesh) {
            return walletTypes.some((type) => type === WalletType.inskech);
        }

        return walletTypes.some((type) => type !== WalletType.vnj && type !== WalletType.inskech);
    }
