...
export class GetWalletAnalyticsUseCase {
  ...
  public async execute(
    getWalletAnalyticsDto: GetWalletAnalyticsDto,
    ...
    const analyticsItems: WalletAnalyticsItem[] = await Promise.all(
      wallets.map(async (wallet) => {
        ...
        operationEntries.forEach((entry) => {
          ...
          if (
            !lastOperationDate ||
            entry.operation.createdAt > lastOperationDate
          ) {
    ...
    const totalBalance = analyticsItems.reduce(
      (sum, item) => sum + item.currentBalance,
      0,
    );
    const totalComing = analyticsItems.reduce(
      (sum, item) => sum + item.coming,
      0,
    );
    const totalExpenditure = analyticsItems.reduce(
      (sum, item) => sum + item.expenditure,
      0,
    );
    const totalNetFlow = totalComing - totalExpenditure;
    const totalOperations = analyticsItems.reduce(
      (sum, item) => sum + item.operationsCount,
      0,
    );
