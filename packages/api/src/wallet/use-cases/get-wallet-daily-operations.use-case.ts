...
export class GetWalletDailyOperationsUseCase {
  ...
  public async execute(
    walletId: string,
    dto: GetWalletDailyOperationsDto,
  ): Promise<GetWalletDailyOperationsOutput> {
    ...
    const allOperations: OperationEntryWithDetails[] = rawOperations.map(
      (op) => ({
        id: op.id,
        amount: op.amount,
        ...
        operationType: op.operation.type
          ? { name: op.operation.type.name }
          : null,
      }),
    );

    const operationsByDate = this.groupOperationsByDate(allOperations);
    const dailyGroups = Array.from(operationsByDate.entries()).map(
      ([date, operations]) => ({
        date,
        operations,
      }),
    ...
    const dailyOperations = await this.calculateDailyBalances(
      paginatedDays,
      walletId,
    );
  ...
  private groupOperationsByDate(
    operations: OperationEntryWithDetails[],
  ): Map<string, DailyOperationResponse[]> {
  ...
  private async calculateDailyBalances(
    ...
    for (const group of dailyGroups) {
      ...
      const dayStartBalance = await this.getBalanceBeforeDate(
        walletId,
        dayStart,
      );
  ...
  private async getBalanceBeforeDate(
    walletId: string,
    date: Date,
  ): Promise<number> {
  ...
  private getDateRange(operations: OperationEntryWithDetails[]): {
    startDate: string;
    endDate: string;
  } {
    ...
    const dates = operations.map(
      (op) => op.createdAt.toISOString().split('T')[0],
    );
