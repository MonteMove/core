...
class WalletDetailsDto {
    ...
    @ApiProperty({ description: 'Полное имя владельца', example: 'Иван Иванов', required: false })
    @IsOptional()
    ...
    @ApiProperty({ description: 'Адрес кошелька', example: '0x1234567890abcdef', required: false })
    @IsOptional()
    @IsString({ message: 'Адрес должен быть строкой' })
    @MaxLength(512, { message: 'Адрес не должен превышать 512 символов' })
    public address?: string;

    @ApiProperty({ description: 'Идентификатор аккаунта', example: 'account-123', required: false })
    @IsOptional()
    @IsString({ message: 'Идентификатор аккаунта должен быть строкой' })
    @MaxLength(255, { message: 'Идентификатор аккаунта не должен превышать 255 символов' })
    public accountId?: string;

    @ApiProperty({ description: 'Имя пользователя', example: 'nickname', required: false })
    @IsOptional()
    @IsString({ message: 'Имя пользователя должно быть строкой' })
    @MaxLength(255, { message: 'Имя пользователя не должно превышать 255 символов' })
    public username?: string;

    @ApiProperty({ description: 'UID биржи', example: 'uid-123', required: false })
    @IsOptional()
    ...
    @ApiProperty({ description: 'ID сети', example: '123e4567-e89b-12d3-a456-426614174000', required: false })
    @IsOptional()
    @IsUUID('4', { message: 'ID сети должен быть валидным UUID' })
    public networkId?: string;

    @ApiProperty({ description: 'ID типа сети', example: '123e4567-e89b-12d3-a456-426614174001', required: false })
    @IsOptional()
...
export class CreateWalletDto {
    ...
    @ApiProperty({ description: 'Детали кошелька', required: false, type: WalletDetailsDto })
    @IsOptional()
    @ValidateNested()
