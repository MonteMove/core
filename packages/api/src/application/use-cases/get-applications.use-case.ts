...
export class GetApplicationsUseCase {
  ...
  public async execute(
    ...
    const applications =
      await this.prisma.application.findMany(findManyOptions);

    const applicationsResponse = applications.map(
      ({ deleted: _, ...application }) => application,
    );
