...
export class NetworkController {
    ...
    @ApiOperation({
        ...
        description: 'Создаёт новую блокчейн-сеть. Доступно администраторам и модераторам.',
    })
    @ApiBody({ type: CreateNetworkDto })
    @ApiResponse({ status: 201, description: 'Сеть успешно создана', type: CreateNetworkResponseDto })
    @ApiCrudResponses()
    public async createNetwork(
        @Body() createNetworkDto: CreateNetworkDto,
        @CurrentUserId() userId: string,
    ): Promise<CreateNetworkResponseDto> {
        const result = await this.createNetworkUseCase.execute(createNetworkDto, userId);
    ...
    @ApiOperation({
        ...
        description: 'Возвращает список сетей с поддержкой фильтрации, поиска и пагинации.',
    })
    @ApiListParams('Фильтры, поиск и пагинация для списка сетей')
    @ApiResponse({ status: 200, description: 'Список сетей успешно получен', type: GetNetworksResponseDto })
    @ApiReadResponses()
    public async getNetworks(@Query() getNetworksDto: GetNetworksDto): Promise<GetNetworksResponseDto> {
    ...
    @ApiResponse({ status: 200, description: 'Сеть найдена', type: NetworkResponseDto })
    @ApiReadResponses()
    public async getNetworkById(@Param('id') networkId: string): Promise<NetworkResponseDto> {
    ...
    @ApiOperation({
        ...
        description: 'Обновляет данные существующей блокчейн-сети. Доступно администраторам и модераторам.',
    })
    @ApiIdParam('Уникальный идентификатор сети')
    @ApiBody({ type: UpdateNetworkDto })
    @ApiResponse({ status: 200, description: 'Сеть успешно обновлена', type: UpdateNetworkResponseDto })
    @ApiCrudResponses()
    public async updateNetwork(
        ...
        const result = await this.updateNetworkUseCase.execute(networkId, updateNetworkDto, userId);
    ...
    @ApiOperation({
        ...
        description: 'Выполняет мягкое удаление сети. Доступно только администраторам.',
    })
    @ApiIdParam('Уникальный идентификатор сети')
    @ApiResponse({ status: 200, description: 'Сеть успешно удалена', type: DeleteNetworkResponseDto })
    @ApiReadResponses()
