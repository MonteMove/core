...
export class OperationTypeController {
  ...
  @ApiOperation({
    ...
    description:
      'Создаёт новый тип операции. Доступно администраторам и модераторам.',
  })
  ...
  public async createOperationType(
    ...
    const result = await this.createOperationTypeUseCase.execute(
      createOperationTypeDto,
      userId,
    );
  ...
  @ApiOperation({
    ...
    description:
      'Возвращает список типов операций с поддержкой фильтрации, поиска и пагинации.',
  })
  ...
  public async getOperationTypes(
    ...
    const result =
      await this.getOperationTypesUseCase.execute(getOperationTypesDto);
  ...
  @ApiReadResponses()
  public async getOperationTypeById(
    @Param('id') operationTypeId: string,
  ): Promise<OperationTypeResponseDto> {
    const result =
      await this.getOperationTypeByIdUseCase.execute(operationTypeId);
  ...
  @ApiOperation({
    ...
    description:
      'Обновляет данные существующего типа операции. Доступно администраторам и модераторам.',
  })
  @ApiIdParam('Уникальный идентификатор типа операции')
  ...
  public async updateOperationType(
    ...
    const result = await this.updateOperationTypeUseCase.execute(
      operationTypeId,
      updateOperationTypeDto,
      userId,
    );
  ...
  @ApiOperation({
    ...
    description:
      'Выполняет мягкое удаление типа операции. Доступно только администраторам.',
  })
  @ApiIdParam('Уникальный идентификатор типа операции')
  @ApiResponse({
    status: 200,
  ...
  public async deleteOperationType(
    ...
    const result = await this.deleteOperationTypeUseCase.execute(
      operationTypeId,
      userId,
    );
