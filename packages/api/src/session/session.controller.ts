...
import { ApiCrudResponses, ApiPaginationParams, ApiReadResponses, ApiTag } from '../common';
...
export class SessionController {
    ...
    @ApiOperation({
        ...
        description: 'Возвращает список всех сессий с поддержкой фильтрации и пагинации',
    })
    @ApiPaginationParams()
    @ApiQuery({ name: 'userId', required: false, description: 'Фильтр по ID пользователя' })
    @ApiQuery({ name: 'sortField', required: false, description: 'Поле сортировки', enum: SessionSortField })
    @ApiQuery({ name: 'sortOrder', required: false, description: 'Направление сортировки', enum: SortOrder })
    @ApiQuery({
        name: 'revoked',
        required: false,
    ...
    @ApiQuery({ name: 'ip', required: false, description: 'Фильтр по IP адресу', example: '192.168.0.1' })
    @ApiResponse({ status: 200, description: 'Список сессий успешно получен', type: GetSessionsResponseDto })
    @ApiReadResponses()
    public async getSessions(@Query() getSessionsDto: GetSessionsDto): Promise<GetSessionsResponseDto> {
    ...
    @ApiQuery({ name: 'ip', required: false, description: 'Фильтр по IP адресу', example: '192.168.0.1' })
    @ApiResponse({ status: 200, description: 'Мои сессии успешно получены', type: GetSessionsResponseDto })
    @ApiReadResponses()
    ...
    public async deactivateUserSessions(
        ...
        const result = await this.deactivateUserSessionsUseCase.execute(userId, excludeSessionId);
    ...
    @ApiResponse({ status: 200, description: 'Мои сессии деактивированы', type: DeactivateSessionsResponseDto })
    @ApiCrudResponses()
    @ApiBody({ type: DeactivateSessionsDto, required: false })
    ...
    @ApiResponse({ status: 200, description: 'Сессия деактивированна', type: DeactivateMySessionByIdResponseDto })
    @ApiCrudResponses()
    public async deactivateMySession(
        @CurrentUserId() userId: string,
        @Param() params: DeactivateMySessionsByIdDto,
    ): Promise<DeactivateMySessionByIdResponseDto> {
        const result = await this.deactivateMySessionByIdUseCase.execute(userId, params.sessionId);
