...
export class NetworkTypeController {
    ...
    @ApiOperation({
        ...
        description: 'Восстанавливает удалённый тип сети. Доступно только администраторам.',
    })
    @ApiIdParam('Уникальный идентификатор типа сети')
    @ApiResponse({
        status: 200,
    ...
    public async restoreNetworkType(
        ...
        const result = await this.restoreNetworkTypeUseCase.execute(networkTypeId, userId);
