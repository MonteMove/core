...
export class NetworkTypeController {
    ...
    @ApiOperation({
        ...
        description: 'Создаёт тип сети для выбранной блокчейн-сети. Доступно администраторам и модераторам.',
    })
    @ApiBody({ type: CreateNetworkTypeDto })
    @ApiResponse({ status: 201, description: 'Тип сети успешно создан', type: CreateNetworkTypeResponseDto })
    @ApiCrudResponses()
    public async createNetworkType(
        @Body() createNetworkTypeDto: CreateNetworkTypeDto,
        @CurrentUserId() userId: string,
    ): Promise<CreateNetworkTypeResponseDto> {
        const result = await this.createNetworkTypeUseCase.execute(createNetworkTypeDto, userId);
    ...
    @ApiOperation({
        ...
        description: 'Возвращает список типов сетей с поддержкой фильтров, поиска и пагинации.',
    })
    @ApiListParams('Фильтры, поиск и пагинация для списка типов сетей')
    @ApiResponse({ status: 200, description: 'Список типов сетей успешно получен', type: GetNetworkTypesResponseDto })
    @ApiReadResponses()
    public async getNetworkTypes(@Query() getNetworkTypesDto: GetNetworkTypesDto): Promise<GetNetworkTypesResponseDto> {
        const result = await this.getNetworkTypesUseCase.execute(getNetworkTypesDto);
    ...
    @ApiResponse({ status: 200, description: 'Тип сети найден', type: NetworkTypeResponseDto })
    @ApiReadResponses()
    public async getNetworkTypeById(@Param('id') networkTypeId: string): Promise<NetworkTypeResponseDto> {
    ...
    @ApiOperation({
        ...
        description: 'Обновляет данные существующего типа сети. Доступно администраторам и модераторам.',
    })
    @ApiIdParam('Уникальный идентификатор типа сети')
    @ApiBody({ type: UpdateNetworkTypeDto })
    @ApiResponse({ status: 200, description: 'Тип сети успешно обновлён', type: UpdateNetworkTypeResponseDto })
    @ApiCrudResponses()
    public async updateNetworkType(
        ...
        const result = await this.updateNetworkTypeUseCase.execute(networkTypeId, updateNetworkTypeDto, userId);
    ...
    @ApiOperation({
        ...
        description: 'Выполняет мягкое удаление типа сети. Доступно только администраторам.',
    })
    @ApiIdParam('Уникальный идентификатор типа сети')
    @ApiResponse({ status: 200, description: 'Тип сети успешно удалён', type: DeleteNetworkTypeResponseDto })
    @ApiReadResponses()
    public async deleteNetworkType(
        @Param('id') networkTypeId: string,
        @CurrentUserId() userId: string,
    ): Promise<DeleteNetworkTypeResponseDto> {
        const result = await this.deleteNetworkTypeUseCase.execute(networkTypeId, userId);
