...
export class GetNetworkTypesUseCase {
    ...
    public async execute(getNetworkTypesDto: GetNetworkTypesDto): Promise<GetNetworkTypesResponse> {
        const {
            search,
            networkId,
            code,
        ...
        const networkTypes = await this.prisma.networkType.findMany(findManyOptions);

        const networkTypesResponse = networkTypes.map(({ deleted: _, ...networkType }) => networkType);
        ...
        return {
            networkTypes: networkTypesResponse,
