  ...
  public async createInitialAdmin(): Promise<void> {
    try {
      ...
      if (!adminUsername || !adminPassword) {
        this.logger.warn(
          'ADMIN_USERNAME ил ADMIN_PASSWORD не предоставлены, пропуск создания админа',
        );
      ...
      if (existingUser) {
        this.logger.warn(
          `Пользователь с username "${adminUsername}" уже существует, пропуск создания`,
        );
      ...
      this.logger.log(
        `Админ пользователь успешно создан: ${adminUser.username}`,
      );
    } catch (error) {
    ...
    try {
      ...
      for (const roleData of rolesToCreate) {
        ...
        if (!existingRole) {
          ...
          this.logger.log(
            `Роль "${roleData.name}" (${roleData.code}) уже существует`,
          );
        }
      }
