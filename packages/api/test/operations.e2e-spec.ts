...
describe('Operation use cases integration', () => {
  ...
  beforeAll(() => {
    ...
    adjustmentOperationUseCase = new AdjustmentOperationUseCase(
      prisma,
      walletRecalculationService,
    );
  ...
  it('creates operation and recalculates wallet balance', async () => {
    ...
    const reloadedWallet = await prisma.wallet.findUniqueOrThrow({
      where: { id: wallet.id },
    });
  ...
  it('returns operation by id with relations', async () => {
    ...
    const operation = await getOperationByIdUseCase.execute(
  ...
  it('throws when requesting deleted operation by id', async () => {
    ...
    await expect(
      getOperationByIdUseCase.execute(created.operation.id),
    ).rejects.toThrow('Операция не найдена');
  });
  ...
  it('lists operations with filters and pagination', async () => {
    ...
    const searchFiltered = await getOperationsUseCase.execute({
      search: 'salary',
    });
    ...
    const conversionFiltered = await getOperationsUseCase.execute({
      conversionGroupId: 10,
    });
    ...
    const typeFiltered = await getOperationsUseCase.execute({
      typeId: secondType.id,
    });

    expect(typeFiltered.operations).toHaveLength(1);
    expect(typeFiltered.operations[0].description).toBe(
      'Secondary wallet income',
    );
...
describe('Operation controller e2e', () => {
  ...
  it('updates and retrieves operation through controller flow', async () => {
    ...
    const walletAfterUpdate = await prisma.wallet.findUniqueOrThrow({
      where: { id: base.wallet.id },
    });
  ...
  it('lists and filters operations via controller, handles deletion and adjustment', async () => {
    ...
    const listAllResponse = await request(server)
      .get('/operations')
      .expect(200);
    ...
    const walletFilterResponse = await request(server)
      ...
      .expect(200);
    const walletFilterBody =
      walletFilterResponse.body as GetOperationsResponseDto;
    ...
    const amountFilterResponse = await request(server)
      ...
      .expect(200);
    const amountFilterBody =
      amountFilterResponse.body as GetOperationsResponseDto;
    ...
    expect(paginatedBody.pagination).toMatchObject({
      total: 3,
      page: 1,
      limit: 2,
      totalPages: 2,
    ...
    await request(server)
      .delete(`/operations/${hardwareBody.operation.id}`)
      .expect(200);
    ...
    const afterDeleteBody =
      afterDeleteResponse.body as GetOperationsResponseDto;
    ...
    const adjustmentBody =
      adjustmentResponse.body as AdjustmentOperationResponseDto;

    expect(adjustmentBody.newAmount).toBe(100);
    const walletAfterAdjustment = await prisma.wallet.findUniqueOrThrow({
      where: { id: base.wallet.id },
    });
...
function createAuthGuard(getUser: () => TestUserPayload | null): CanActivate {
  return {
    canActivate: (context) => {
      ...
      const req = context
        .switchToHttp()
        .getRequest<Request & { user?: TestUserPayload }>();
